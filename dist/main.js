...
(function(PullRequestState2) {
  PullRequestState2["Pending"] = "PENDING";
  PullRequestState2["ChangesRequested"] = "CHANGES_REQUESTED";
  PullRequestState2["Commented"] = "COMMENTED";
  PullRequestState2["Dismissed"] = "DISMISSED";
  PullRequestState2["Approved"] = "APPROVED";
})(PullRequestState || (PullRequestState = {}));
var getRequestedReviewers = async (kit, context2) => {
  ...
  response.data.users.forEach((user) => {
    if (user.login === context2.payload.pull_request.user.login) {
      return;
    }
  ...
  for (let team of response.data.teams) {
    const response2 = await kit.teams.listMembersInOrg({
      org: context2.repo.owner,
      team_slug: team.name
    });
    ...
    response2.data.forEach((user) => {
      ...
      users2.push(user.login);
    });
    if (users2.length) {
      teams.set(team.name, users2);
    }
  }
  return {
    users,
...
var decideByState = (reviewer, state) => {
  switch (state) {
    case PullRequestState.Pending:
      (0, import_core.setFailed)(`There is a pending review from ${reviewer}`);
      break;
    case PullRequestState.ChangesRequested:
      (0, import_core.setFailed)(`Please implement the requested changes or dismiss the review from ${reviewer}`);
      break;
    case PullRequestState.Commented:
      (0, import_core.setFailed)(`There is a pending approval from ${reviewer}`);
      break;
    case PullRequestState.Dismissed:
      (0, import_core.setFailed)(`Dismissed review from ${reviewer}, re-request a review & wait for approval`);
      break;
...
async function run() {
  try {
    if (import_github.context.eventName !== "pull_request" && import_github.context.eventName !== "pull_request_review") {
      (0, import_core.setFailed)(`Invalid event: ${import_github.context.eventName}, it should be use on pull_request or pull_request_review`);
      return;
    }
    const token = (0, import_core.getInput)("token");
    const kit = (0, import_github.getOctokit)(token);
    const requestedReviewers = await getRequestedReviewers(kit, import_github.context);
    const actualReviewers = await getReviewers(kit, import_github.context);
    actualReviewers.forEach((state, reviewer) => {
    ...
    requestedReviewers.users.forEach((reviewer) => {
      if (!actualReviewers.has(reviewer)) {
        (0, import_core.setFailed)(`Waiting for review from ${reviewer}`);
      }
      decideByState(reviewer, actualReviewers.get(reviewer));
    });
    requestedReviewers.teams.forEach((reviewers, team) => {
      const reviewersApprovals = reviewers.filter((reviewer) => actualReviewers.has(reviewer));
      if (!reviewersApprovals.length) {
        (0, import_core.setFailed)(`Waiting for review from any member from ${team} (${reviewers.join(", ")})`);
      }
      reviewersApprovals.forEach((reviewer) => {
